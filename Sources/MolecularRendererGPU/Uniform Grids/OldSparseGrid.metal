...
namespace old_sparse_grids_draft {
  ...
  struct SparseCPUInputs {
    ...
    ushort current_far_width;
    ushort current_close_width;
    command_buffer command_buffer;
    compute_pipeline_state pass_2_pipeline;
    compute_pipeline_state pass_4_pipeline;
    // Another stage for freeing unused memory pages, done after pass 4.
    compute_pipeline_state memset_pattern4_pipeline;
  };
  
  struct SparseCPUOutputs {
    ushort out_of_memory;
    uint requested_memory;
  };
  ...
  // 16 KB
  // (8 x 256) x 4 references @ 0.5 nm
  // (8 x 64) x 16 references @ 0.25 nm
  ...
  public:
    // TODO: Remove the "is high res" heuristic.
  ...
  // will encode command 3.
  //
  // Write the camera's voxel into args for pass 2 (the last frame wrote bounds).
  //
  // Preserve atom data from the previous frame.
  
  // The GPU should perform garbage collection using mark-and-sweep over pages of
  // supermassive allocations. There is no triple-buffering for the BVH.
  kernel void sparse_grid_pass1
  (
   device uchar *heap [[buffer(0)]],
   constant SparseCPUInputs &inputs [[buffer(1)]],
   device SparseGridArguments *pass_2_args [[buffer(2)]])
  {
    // Other simds in the threadgroup will encode commands to clear buffers, etc.
  }
  
  // Sort atoms into upper voxels.
  // TODO: Write into a series of atomics, one per GPU core.
  // TODO: Use 128-threadgroups.
...
#define object (upper_voxel_offsets + address)
  ...
  // Allocate memory for lower voxels.
  // TODO: If you ran out of memory, copy the last frame's heap and disable high
  // res.
  // TODO: If no BVH was built yet, the RT shader just renders an empty grid.
  // TODO: If in production animation mode, cause CPU-side stalls instead of
  // rendering anything.
  kernel void sparse_grid_pass3
  (
   constant SparseGridArguments &args [[buffer(0)]],
   
   device Arguments *shader_args [[buffer(20)]])
  {
    // Dispatch an entire 384-threadgroup, which sums partial outputs from pass 1.
   ...
   ushort lane_id [[thread_index_in_simdgroup]])
  {
    ...
    if (equality_succeeded) {
      ...
      for (; atom_offset < atom_end; atom_offset += tg_size) {
        ...
        // TODO: Don't copy the atoms beyond a certain cutoff point (instead,
        // store a pointer in memory to somewhere in the current frame's input
        // atom buffer). This makes accesses more divergent, but accomodate by
        // reducing the AO sample count.
      }
      return;
    }
