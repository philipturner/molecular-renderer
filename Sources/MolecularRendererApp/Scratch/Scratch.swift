...
func createGeometry() -> [Entity] {
  ...
  // Minimize the scene and run an ambient MD simulation. Next,
  // animate how the logic rods are expected to move with each I/O pair and
  // reversible clocking. Ensure there is no geometric overlap. Finally, test
  // the forward execution pass of two I/O pairs with an MD simulation. One
  // pair outputs 0; another outputs 1.
  
  var output: [Entity] = []
  for housingUnit in xorGate.housingUnits {
    output += housingUnit.topology.atoms
  }
  for logicRod in xorGate.logicRods {
    output += logicRod.topology.atoms
  }
  return output
}
