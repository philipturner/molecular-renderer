...
//  var provider: ArrayAtomProvider
  ...
  init() {
    ...
    // Not yet a plane, but able to be transformed into a plane.
    var _100Removers: [[(SIMD3<Float>, SIMD3<Float>)]] = []
    
    // Start rewriting by systematically mutating different (100) planes of the
    // cube.
    do {
      // This loop probably needs to extend to double the width, but check
      // first.
      for i in 0...latticeWidth * 4 {
        let origin = SIMD3<Float>(0, Float(i) * 4, 0)
        _100Removers.append([(origin, SIMD3(1, 1, 1)),
                             (origin, SIMD3(1, -1, -1))])
      }
      for i in 0...latticeWidth * 4 {
        let origin = SIMD3<Float>(0, 0, Float(i) * 4)
        _100Removers.append([(origin, SIMD3(1, 1, -1)),
                             (origin, SIMD3(1, -1, 1))])
      }
    }
    ...
    func remove100(
      ...
      planes: [Plane],
      axis: Int,
      locationOnAxis: Float,
      positiveOnAxis: Bool
    ) -> [Cell] {
      precondition(axis >= 0 && axis <= 2, "Invalid axis.")
      
      var allRemovers: [[Plane]] = []
      for var _100Remover in _100Removers {
        var removers: [Plane] = []
        for i in 0..<2 {
          _100Remover[i].0.x = locationOnAxis
          if !positiveOnAxis {
            _100Remover[i].0.x.negate()
          }
          
          func permuteVector(_ vector: SIMD3<Float>) -> SIMD3<Float> {
            if axis == 0 {
              return vector
            } else if axis == 1 {
              return SIMD3(vector.y, vector.x, vector.z)
            } else {
              return SIMD3(vector.y, vector.z, vector.x)
            }
          }
          _100Remover[i].0 = permuteVector(_100Remover[i].0)
          _100Remover[i].1 = permuteVector(_100Remover[i].1)
          removers.append(Plane(_100Remover[i].0, normal: _100Remover[i].1))
        }
        allRemovers.append(removers)
      }
      
      var desiredLocationOnAxis = locationOnAxis
      if !positiveOnAxis {
        desiredLocationOnAxis -= 1
        precondition(desiredLocationOnAxis >= 0)
      }
     
      var output = cells
      for i in cells.indices {
        // Enable this optimization after debugging.
        #if false
        if Float(cells[i].offset[axis]) != desiredLocationOnAxis {
          continue
        }
        #endif
        
//        for remover in [allRemovers[5], allRemovers[6]] {
        for remover in allRemovers {
          output[i] = output[i].cleaved(planes: remover)
        }
      }
    ...
    do {
      ...
      let latticeCenter: Float = Float(latticeWidth) / 2
      let latticeLength: Float = Float(latticeWidth)
      precondition(latticeCenter == rint(latticeCenter))
      
      let housingThickness: Float = 0.5
      let innerYZ_left: Float = 2
      let innerYZ_right: Float = 4
      let outerYZ_left: Float = innerYZ_left + housingThickness
      let outerYZ_right: Float = innerYZ_right + housingThickness
      
      let holeThicknessX: Float = 1
      let holeThicknessYZ: Float = 1
      let holeBoundsX: [SIMD2<Float>] = [
        SIMD2(
          0 + holeThicknessX,
          latticeCenter - holeThicknessX / 2
        ),
        SIMD2(
          latticeCenter + holeThicknessX / 2,
          latticeLength - holeThicknessX
        ),
      ]
      let holeYZ: Float = innerYZ_right - holeThicknessYZ
      
      cells = remove100(cells: cells, planes: [], axis: 0, locationOnAxis: 5.5, positiveOnAxis: true)
...
//    self.provider = ArrayAtomProvider(allAtoms)
    ...
    diamondoid.removeCloseHydrogens()
