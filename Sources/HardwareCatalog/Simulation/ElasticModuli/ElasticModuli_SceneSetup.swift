...
func createGeometry() -> [[Entity]] {
  ...
  // See whether this simulation can be accelerated with rigid body mechanics
  // (timestep 40 fs, only moving rigid body for specimen). Delete the code
  // afterward.
  var output: [[Entity]] = []
  for frameID in 0..<480 {
    let timeStep: Double = 0.040
    if frameID % 10 == 0 {
      print("frame=\(frameID), time=\(String(format: "%.3f", timeStep * Double(frameID)))")
    }
