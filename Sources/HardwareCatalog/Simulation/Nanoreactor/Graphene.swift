...
//  MolecularRenderer
//
//  Created by Philip Turner on 3/1/24.
//
...
func createGeometry() -> [[Entity]] {
  ...
  let initialAtoms = graphene.topology.atoms
  ...
  var velocities = [SIMD3<Float>](
    repeating: .zero, count: graphene.topology.atoms.count)
  for frameID in 0...120 {
    print("frame", frameID)
    
    if frameID > 0 {
      updateMolecule(env: env, mol: mol, atoms: graphene.topology.atoms)
      
      let forces = createForces(
        env: env, mol: mol, calc: calc, res: res,
        atomCount: graphene.topology.atoms.count)
      let masses = createMasses(atoms: graphene.topology.atoms)
      let anchors = Set(graphene.anchors)
      
      for atomID in graphene.topology.atoms.indices {
        var momentum = velocities[atomID] * masses[atomID]
        momentum += 0.002 * forces[atomID]
        var velocity = momentum / masses[atomID]
        ...
        if anchors.contains(UInt32(atomID)) {
          velocities[atomID] = .zero
        } else {
          velocities[atomID] = velocity
          graphene.topology.atoms[atomID].position += 0.002 * velocity
        }
      }
    }
    
    output.append(graphene.topology.atoms)
  }
  return output
}
